window.dom = {
    ...
    attr (node, key, values) {
        if (arguments.length === 2) return node.getAttribute(key);
        else if (arguments.length === 3) node.setAttribute(key, values);
    },
    text (node, string) {
    ...
    html (node, string) {
        ...
        else if (arguments.length === 2) node.innerHTML = string;
    },
    ...
    each (nodes, fn) {
        for(let i = 0; i < nodes.length; i++)fn.call(null, nodes[i]);
    },
    index (node) {
        let i;
